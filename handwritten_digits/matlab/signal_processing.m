...
%  t=[1 0 0 0 1;
%     1 0 0 0 1];
A = importdata('test_012.txt');
k = A.';
B = importdata('test_lable_012.txt');
t = B.';
...
%%
% Plot figure
X = importdata('test_number1.txt');
% X = C.';
z2 = w2 * X + b2;			        % input weight for hidden layer *ones(1,size(X,2))
a2= 1./(1+exp(-z2));                            	% output for hidden layer
z3 = w3 * a2 + b3*ones(1,size(X,2));			    % input weight for output layer
a3= 1./(1+exp(-z3));                            	% output for output layer
% D = importdata('test_data_lable.txt');       
% figure(1);
% plot(t4,D);
